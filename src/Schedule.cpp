...
void Schedule::addSession(int day, int startTime, int endTime, const std::string& venue, Event* event) {
    ...
    for (const auto& athlete : event->getParticipants()) {
        athlete_busy_slots[athlete->getId()][day].push_back({startTime, endTime});
    }
}
...
bool Schedule::isAthleteAvailable(const Athlete* athlete, int day, int startTime, int endTime) const {
    ...
    if (athlete_busy_slots.count(athlete->getId()) && athlete_busy_slots.at(athlete->getId()).count(day)) {
        for (const auto& busy_slot : athlete_busy_slots.at(athlete->getId()).at(day)) {
