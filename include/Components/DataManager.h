...
// 数据管理器 (负责数据持久化，如备份和恢复)
/**
 * @brief 管理系统数据的持久化，包括备份、恢复和加载示例数据。
 *
 * DataManager 依赖 SystemSettings 对象协作来访问和修改系统中的核心数据，
 * 包括单位、运动员、比赛项目、成绩和积分管理等。
 * 它提供了将当前系统状态保存到文件以及从文件加载数据的功能。
 * 同时，它也负责一个用于开发和系统测试的示例数据集加载机制。
 */
class DataManager {
private:
...
public:
     ...
     * @param sysSettings 对 SystemSettings 对象的引用，数据管理器将基于此设置进行操作。
     */
     ...
     *
     * 备份数据通常包括单位、运动员、比赛项目、预设的计分规则、已记录的成绩等。
     * 数据的序列化格式（如JSON, XML, 或自定义）由具体实现决定。
     * @param filePath 用于备份备份数据的文件路径。
     ...
     * @brief 从指定的文件恢复系统数据。
     *
     * 此操作将覆盖当前系统中的大部分数据（除默认设置，如计分规则），
     * 然后从文件中加载并重构单位、运动员、项目、成绩等。
    ...
    /**
     * @brief 向当前系统中加载一组预定义的示例数据。
     *
     * 此方法用于开发或演示系统的功能测试或展示。
     * 它会添加一些示例单位、运动员、比赛项目以及它们之间的报名关系。
     * @note 此方法不会覆盖已初始化的默认计分规则
     * @return 如果示例数据集加载成功，返回 true；否则返回 false。
     */
    bool loadSampleData();
    
    /**
     * @brief 加载阶段1的示例数据（单位、场地、计分规则）
     * @return 如果加载成功，返回 true；否则返回 false。
     */
    bool loadSampleStage1Data();
    
    /**
     * @brief 加载阶段2的示例数据（运动员及其报名信息）
     * @return 如果加载成功，返回 true；否则返回 false。
     */
    bool loadSampleStage2Data();
    
    /**
     * @brief 加载阶段3的示例数据（比赛成绩）
     * @return 如果加载成功，返回 true；否则返回 false。
     */
